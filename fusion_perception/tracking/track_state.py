...
class TrackState:
    ...
    def to_track(self) -> Track:
        ...
        if self.cow_points_abs is not None:
            ...
            cow_qp = [0., 0.]
        # Reorder KF state [cx,cy,cz,θ,l,w,h] → box_3d [cx,cy,cz,w,h,l,ry]
        box3d = None
        if self.last_box3d is not None:
            s = self.last_box3d
            box3d = [float(s[0]), float(s[1]), float(s[2]),
                     float(s[5]), float(s[6]), float(s[4]), float(s[3])]
        vel = self.kf.x[7:10, 0].tolist()
        return Track(
            ...
            last_box_2d=list(self.last_bbox2d) if self.last_bbox2d is not None else None,
            last_box_3d=box3d,
            velocity_3d=vel,
        )
