...
def init_kf(box3d: list[float], dt: float = 0.1) -> KalmanFilter:
    ...
    # Process noise: cz/vz larger to model monocular depth noise
    kf.Q = np.diag([
        0.1, 0.1, 0.5,       # cx, cy, cz  (cz raised: monocular depth noisy)
        0.05,                 # theta
        0.02, 0.02, 0.02,    # l, w, h
        2.0, 2.0, 4.0,       # vx, vy, vz  (vz raised for forward-speed variance)
    ]).astype(np.float64)

    # Initial state covariance: high uncertainty on velocities and depth
    kf.P = np.diag([
        1.0, 1.0, 4.0,       # cz raised: monocular depth has ~±2m uncertainty at spawn
        0.1,
        0.5, 0.5, 0.5,
        10.0, 10.0, 10.0,
    ]).astype(np.float64)
